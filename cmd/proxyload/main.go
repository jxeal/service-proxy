...
// TEST-ONLY: flags for load testing (defaults target local backends on 9001/9002/9003)
var (
	backendsFlag = flag.String("backends", "http://127.0.0.1:9001,http://127.0.0.1:9002,http://127.0.0.1:9003", "comma-separated backend URLs") // TEST-ONLY
	scanFlag     = flag.Duration("scan", 5*time.Second, "scan interval")                                                                          // TEST-ONLY
	poolFlag     = flag.Bool("pool", false, "enable pooled transport")                                                                            // TEST-ONLY
	nologFlag    = flag.Bool("nolog", false, "disable routing log")                                                                               // TEST-ONLY
)
...
func NewSmartProxy(registry *Registry) *httputil.ReverseProxy {
	...
	return &httputil.ReverseProxy{
		...
		ErrorHandler: func(w http.ResponseWriter, r *http.Request, err error) {
			log.Printf("proxy error path=%s err=%v", r.URL.Path, err) // TEST-ONLY
...
func main() {
	flag.Parse() // TEST-ONLY
	// 1. Setup Signal handling for Graceful Shutdown
	ctx, stop := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM)
	defer stop()

	// 2. Initialize Registry with test targets // TEST-ONLY
	targets := strings.Split(*backendsFlag, ",") // TEST-ONLY
	registry := NewRegistry(targets)             // TEST-ONLY
	...
	if *poolFlag { // TEST-ONLY
		proxy.Transport = &http.Transport{MaxIdleConns: 200, MaxIdleConnsPerHost: 100, IdleConnTimeout: 90 * time.Second} // TEST-ONLY
	} // TEST-ONLY
	server := &http.Server{
		Addr:    ":8080",
		Handler: proxy,
	...
	ticker := time.NewTicker(*scanFlag) // TEST-ONLY
	defer ticker.Stop()
